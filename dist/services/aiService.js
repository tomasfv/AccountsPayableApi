...
async function buildContext() {
    ...
    const bills = await models_1.Bill.findAll({
        include: [{ model: models_1.Vendor, as: "vendor" }],
        order: [["dueDate", "ASC"]],
    });
    const payments = await models_1.Payment.findAll({ order: [["createdAt", "DESC"]] });
    const users = await models_1.User.findAll({ order: [["fullName", "ASC"]] });
    const vendors = await models_1.Vendor.findAll({ order: [["name", "ASC"]] });
    const cards = await models_1.Card.findAll({ include: [{ model: models_1.User, as: "creator", attributes: ["fullName"] }] });
...
`;
    for (let i = 0; i < bills.length; i++) {
        ...
        const dueDate = new Date(b.get("dueDate"));
        const dueDateStr = b.get("dueDate");
        const status = b.get("status");
        const amount = parseFloat(String(b.get("amount"))) || 0;
        const inv = b.get("invoiceNumber") || "—";
        const createdById = b.get("createdById");
        const approvedById = b.get("approvedById");
        const fileUrl = b.get("fileUrl");
